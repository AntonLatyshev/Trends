...
class ControllerModuleOCFilter extends Controller {
  ...
  public function __construct($registry) {
    ...
    if (isset($this->request->get['filter_ocfilter'])) {
      ...
      if ($this->params) {
  			$this->options_get = decodeParamsFromString($this->params, $this->config);

        if ($this->config->get('ocfilter_show_price') && !empty($this->options_get['p'])) {
          $range = getRangeParts(end($this->options_get['p']));
    ...
    $this->option_keywords = array();
    $this->value_keywords = array();

    $this->option_keywords['p'] = 'price';
    $this->option_keywords['s'] = 'sklad';
    $this->option_keywords['m'] = 'proizvoditel';

    $results = $this->model_catalog_ocfilter->getManufacturersByCategoryId($this->category_id);

    foreach ($results as $value) {
      if ($value['keyword']) {
      	$this->value_keywords['m-' . $value['value_id']] = $value['keyword'];
      } else {
      	$this->value_keywords['m-' . $value['value_id']] = $value['value_id'];
      }
    }
    ...
    foreach ($results as $result) {
    	$this->pages[md5($result['category_id'] . $result['ocfilter_params'])] = $result['ocfilter_page_id'];
    }
  ...
  public function decode() {
    ...
    foreach ($keywords as $key => $keyword) {
      ...
      if ($keyword == 'price') {
        ...
        $option_id = $this->cache->get('ocfilter.decode.option.' . md5($keyword));
        ...
        if (false === $option_id) {
          ...
          $this->cache->set('ocfilter.decode.option.' . md5($keyword), (int)$option_id);
        }
