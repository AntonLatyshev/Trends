...
class ControllerNewsNcategory extends Controller {  
	...
	protected function getPageContent($settings) {	
		...
		if (isset($this->request->get['ncat'])) {
			...
			$template_name_load = $ncategory_info['template_name'];
		...
		if ($ncategory_info) {
				...
				$data['heading_title'] = $ncategory_info['name'];						
				$data['template_name'] = $ncategory_info['template_name'];
				...
				foreach ($results as $result) {
					$data['ncategories'][] = array(
						'name'  => $result['name'],
						'thumb'  => $this->model_tool_image->resize($result['image'], 380, 300),
		...
		if (isset($this->request->get['author'])) {
			...
			if ($ncategory_info && $ncategory_id != 65) {
				$sdata = array(
			...
			foreach ($results as $result) {
				...
				if (in_array("image", $elements) && ($result['image'] || $result['image2'])) {
					if ($result['image2']) {
						...
						$image = $this->model_tool_image->cropsize($result['image'], $bbwidth, $bbheight);
					}
				} else {
					$image = false;
				}
		...
		if ( !empty($template_name_load) ) {

			if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/news/' . $template_name_load . '.tpl')) {
				return $this->load->view($this->config->get('config_template') . '/template/news/' . $template_name_load . '.tpl', $data);
			} else {
				return $this->load->view('default/template/news/' . $template_name_load . '.tpl', $data);
			}
