...
class ControllerNewsArticle extends Controller {
	...
	protected function getPath($parent_id, $current_path = '') {
		...
		$category_info = $this->model_catalog_ncategory->getncategory($parent_id);

		if ($category_info) {
			if (!$current_path) {
				$new_path = $category_info['ncategory_id'];
			} else {
				$new_path = $category_info['ncategory_id'] . '_' . $current_path;
			}
	...
	protected function getTrueUrl($product_id) {
		$categories = $this->getNcategoriesbyNewsId($product_id);

		if ( $categories ) {

			foreach($categories as $category){
				$path_true = $this->getPath($category['ncategory_id']);
//				$category_info = $this->getncategory($category['ncategory_id']);
				$category_info = $this->model_catalog_ncategory->getncategory($category['ncategory_id']);

				if ( $path_true ) {
					$cat_path = $path_true;
				} else {
					$cat_path = $category_info['ncategory_id'];
				}
...
//					break;
	...
	public function index() {
		...
		if (isset($this->request->get['ncat'])) {
			...
			$this->response->redirect($this->url->link('news/article', '&ncat=' . $this->getTrueUrl($news_id) . '&news_id=' . $news_id), '301');
...
//
//		if(stristr($current_url['path'], 'blog-headlines') === FALSE) {
//			$this->response->redirect($this->url->link('news/article', '&ncat=' . $this->getTrueUrl($news_id) . '&news_id=' . $news_id), '301');
//		}

		// Fixed SEO url
		$path_true = $this->getTrueUrl($news_id);

		if (array_key_exists('search', $this->request->get)) {

		} elseif (array_key_exists('manufacturer_id', $this->request->get)) {

		} elseif (array_key_exists('ncat', $this->request->get)) {
			if (!empty($path_true)) {
				if ($ncat != $path_true) {
					$this->response->redirect($this->url->link('news/article', '&ncat=' . $path_true . '&news_id=' . $this->request->get['news_id']), '301');
				}
			}
		} else {
			if (!empty($path_true)) {
		...
		// выведем верхний баннер для блоговых сущностей хардкодом, по id
		...
		if ($news_info) {
			...
			if (!$this->config->get('ncategory_bnews_tplpick')) {
				if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/news/layout_article.tpl')) {
					$this->response->setOutput($this->load->view($this->config->get('config_template') . '/template/news/layout_article.tpl', $data));
				} else {
					$this->response->setOutput($this->load->view('default/template/news/layout_article.tpl', $data));
				}
			} else {
	...
	protected function getPageContent($news_info) {
		...
		// Текст для кнопки Drop down
		$data['data_short'] = $this->language->get('data_short');
		$data['data_full'] = $this->language->get('data_full');
		...
		$data['button_other_project'] = $this->language->get('button_other_project');
		$data['button_other_project_link'] =  $this->url->link('news/ncategory', 'ncat=61');
		$data['button_other_services'] = $this->language->get('button_other_services');
		$data['button_other_services_link'] =  $this->url->link('news/ncategory', 'ncat=62');
		$data['button_other_project_text'] = $this->language->get('button_other_project_text');
		$data['button_other_services_text'] = $this->language->get('button_other_services_text');
		...
		$data['ncategory_id'] = $catid['ncategory_id'];
		$catinfo_id = $this->model_catalog_ncategory->getncategory($catid['ncategory_id']);
		if ( !empty($catinfo_id['parent_id']) ) {
			$data['ncategory_id'] = $catinfo_id['parent_id'];
		} else {
			$data['ncategory_id'] = $catid['ncategory_id'];
		}
		...
		if ($news_info['image']) {
				$data['thumb'] = $this->model_tool_image->onesize($news_info['image'], $bwidth);
				$data['popup'] = $this->model_tool_image->onesize($news_info['image'], 800);
		} else {
				$data['thumb'] = '';
				$data['popup'] = '';
		}
		...
		foreach ($gal_results as $result) {
			...
			$data['gallery_images'][] = array(
					...
					'thumb_slider' => $this->model_tool_image->resize($result['image'], $news_info['gal_slider_w'], $news_info['gal_slider_h']),
			);
		}
		$data['products'] = array();
