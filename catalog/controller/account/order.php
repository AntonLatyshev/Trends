...
//		);
		...
		$data['column_ttn'] = $this->language->get('column_ttn');
...
//			);
			...
			foreach ($products as $product) {
				...
				if (isset($product_info['image']) && is_file(DIR_IMAGE . $product_info['image'])) {
					$image = $this->model_tool_image->resize($product_info['image'], 62, 62);
				} else {
					$image = $this->model_tool_image->resize('placeholder.png', 62, 62);
				}
			...
			foreach ($totals as $total) {
				if ($total['code'] == 'shipping') {
					$data['total_shipping'] = $this->currency->format($total['value'], $order_info['currency_code'], $order_info['currency_value']);
				}

				if ($total['code'] == 'total') {
					$data['total_total'] = $this->currency->format($total['value'], $order_info['currency_code'], $order_info['currency_value']);
				}
		...
		if ($order_info) {
			...
			if ($order_product_info) {
				...
				if ($product_info) {
					...
					$this->session->data['success'] = sprintf($this->language->get('text_success'), $this->url->link('product/product', 'product_id=' . $product_info['product_id']), $product_info['name'], $this->url->link('checkout/checkout'));
