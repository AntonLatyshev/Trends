		...
		if (isset($this->request->get['_route_'])) {
			...
			foreach ($parts as $part) {
				...
				if ($query->num_rows) {
					...
					if ($query->row['query'] && $url[0] != 'information_id' && $url[0] != 'manufacturer_id' && $url[0] != 'category_id' && $url[0] != 'service_id' && $url[0] != 'product_id') {
						$this->request->get['route'] = $query->row['query'];
					}
				} else {
				    foreach ($this->model_localisation_language->getLanguagesNotMain() as $item) {
                        if ($part == $item['code']) $this->session->data['language'] = $item['code'];
                        $code_array[] = $item['code'];
                    }

                    if(!in_array($part, $code_array)) {
			...
			if (!isset($this->request->get['route'])) {
				if (isset($this->request->get['product_id'])) {
					...
					$this->request->get['route'] = 'product/service';
			...
			if (isset($this->request->get['route']) && $this->request->get['route'] != 'product/product' && $this->request->get['route'] != 'product/category' && $this->request->get['route'] != 'product/service' && $this->request->get['route'] != 'product/manufacturer/info' && $this->request->get['route'] != 'information/information') {
		...
		foreach ($data as $key => $value) {
			if (isset($data['route'])) {
				if (($data['route'] == 'product/product' && $key == 'product_id')
					|| (($data['route'] == 'product/manufacturer/info' || $data['route'] == 'product/product')
						&& $key == 'manufacturer_id')
...
//					$url .=  '/'.$blog_headlines;
					$url .=  '';
				} elseif ($data['route'] == 'product/service' && $key == 'path') {
					$services = explode('_', $value);

					foreach ($services as $service) {
						$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "url_alias WHERE `query` = 'service_id=" . (int)$service . "'");
					...
					$url .= '/contact';
				} elseif (isset($data['route']) && $data['route'] == 'product/special' && $key != 'page'
					&& $key != 'sort'
					&& $key != 'limit'
					&& $key != 'order'
				) {
					...
					$url .= '/manufacturer';
				} elseif (isset($data['route']) && $data['route'] == 'product/vacansies') {
					$url .= '/vacancies';
				}
			}
		}
		...
		if ($url) {
			...
			foreach ($this->model_localisation_language->getLanguagesNotMain() as $item){
                if($this->session->data['language'] == $item['code'] ){
                    $url = '/'.$item['code'].$url;
                }
            }
