	...
	public function index() {
		...
		if ($category_info) {
			...
			if ( ($this->config->get('config_customer_group_id') == $category_info['customer_group_id']) || $category_info['customer_group_id'] == '1' ) {
				...
				if ($category_info['image_bg']) {
					$data['thumb_bg'] = 'image/' . $category_info['image_bg'];
				} else {
					$data['thumb_bg'] = '';
				}
  		...
  		// OCFilter end
				...
				// Акционне товары у категории (для соотв. блока)
				...
				foreach ($results as $result) {
                    if (isset($result['image']) && is_file(DIR_IMAGE . $result['image'])) {
						$image = $this->model_tool_image->resize($result['image'], 212, 200);
						$image_sale = $this->model_tool_image->resize($result['image'], 73, 69);
					} else {
						$image = $this->model_tool_image->resize('placeholder.png', 212, 200);
						$image_sale = $this->model_tool_image->resize('placeholder.png', 73, 69);
					}
					...
					foreach ($product_images as $product_image) {
						if (is_file(DIR_IMAGE . $product_image['image'])) {
							$images[] = $this->model_tool_image->resize($product_image['image'], 212, 200);
						}
					}
				...
				$data['products_sale'] = array_slice($data['products_sale'], 0, 4);
				...
				$limits = array_unique(array($this->config->get('config_product_limit'), 18, 27, 36, 72));
