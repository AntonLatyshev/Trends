...
<!--  </div>-->
  <span class="close-panel"><?=$text_close;?></span>
  <div class="list-group" id="ocfilter-content">
      <?php if ($selecteds) { # Selected options ?>
      <div class="ocfilter-block__selected">
          <span class="ocfilter__params-title"><?=$heading_title;?></span>
          <div class="list-group-item selected-options ocfilter__selected">
            <?php foreach ($selecteds as $option) { ?>
            <div class="ocfilter__selected-option">
              <span><?php echo $option['name']; ?>:</span>
              <?php foreach ($option['values'] as $value) { ?>
              <button type="button" onclick="location = '<?php echo $value['href']; ?>';" class="btn btn-xs btn-danger ocfilter__cancel"><?php echo $value['name']; ?></button>
              <?php } ?>
            </div>
            <?php } ?>
            <?php $count = count($selecteds); $selected = $selecteds; $first = array_shift($selected); ?>
            <?php if ($count > 1 || count($first['values']) > 1) { ?>
            <button type="button" onclick="location = '<?php echo $link; ?>';" class="btn btn-block btn-danger ocfilter__cancel ocfilter__cancel--expanded" style="border-radius: 0;"><i class="fa fa-times-circle"></i> <?php echo $text_cancel_all; ?><em></em></button>
            <?php } ?>
          </div>
      </div>
      <?php } ?>
      ...
      <div class="list-group-item active ocfilter-option">
...
<!--        </div>-->
          <div class="ocfilter-price">
              <span class="ocfilter-price__title"><?=$text_price;?>:</span>
              <div class="ocfilter-price__inputs">
                  <div class="ocfilter-price__input-wrapper">
                      <input name="price[min]" value="<?php echo $min_price_get; ?>" type="text" class="ocfilter-price__input" id="min-price-value"/>
                      <span class="ocfilter-price__cur"><?=$symbol_right;?></span>
                  </div>
                  <div class="ocfilter-price__separartor"></div>
                  <div class="ocfilter-price__input-wrapper">
                      <input name="price[max]" value="<?php echo $max_price_get; ?>" type="text" class="ocfilter-price__input" id="max-price-value"/>
                      <span class="ocfilter-price__cur"><?=$symbol_right;?></span>
                  </div>
              </div>
          </div>
          <div class="option-values price">
              ...
              ></div>
              <div class="hidden" id="ocfilter-button">
                  <button class="show-price-btn btn btn-primary disabled" data-loading-text="<i class='fa fa-refresh fa-spin'></i> Загрузка.."></button>
              </div>
      ...
      <?php } ?>
			<?php } ?>
				...
				<div class="list-group-item active ocfilter-option" id="option-<?php echo $option['option_id']; ?>">
	        <div class="option-name">
...
<!--            <p>-->
<!--              <a class="opener-text" href="#ocfilter-hidden-values---><?php //echo $option['option_id']; ?><!--" data-toggle="collapse" aria-expanded="false" aria-controls="ocfilter-hidden-values">--><?php //echo $text_show_all; ?><!-- <i class="fa fa-angle-down"></i></a>-->
<!--            </p>-->
            <div class="collapse" id="ocfilter-hidden-values-<?php echo $option['option_id']; ?>">
                ...
                <button class="value-cancel"></button><em></em>
                <?php } ?>
              </label>
							<?php } elseif ($value['count']) { ?>
            ...
            <?php if ($show_values_limit && $show_values_limit < count($option['values']) && $key + 1 == count($option['values'])) { ?>
                ...
                <p>
<!--                    <a class="opener-text" href="#ocfilter-hidden-values---><?php //echo $option['option_id']; ?><!--" data-toggle="collapse" aria-expanded="false" aria-controls="ocfilter-hidden-values">--><?php //echo $text_show_all; ?><!-- <i class="fa fa-angle-down"></i></a>-->
                    <a class="opener-text" href="#ocfilter-hidden-values-<?php echo $option['option_id']; ?>" data-toggle="collapse" data-text-show="<?php echo $text_hide; ?>" data-text-hide="<?php echo $text_show_all; ?>"><?php echo $text_show_all; ?></a>
                </p>
						<?php } ?>
