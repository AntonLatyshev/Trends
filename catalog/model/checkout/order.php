...
class ModelCheckoutOrder extends Model {
	...
	public function addOrderHistory($order_id, $order_status_id, $comment = '', $notify = false) {
		...
		if ($order_info) {
			...
			$this->db->query("UPDATE `" . DB_PREFIX . "order` SET customer_id = '" . (int)$customer_id . "', order_status_id = '" . (int)$order_status_id . "', date_modified = NOW() WHERE order_id = '" . (int)$order_id . "'");
			...
			if (!$order_info['order_status_id'] && $order_status_id) {
				...
				foreach ($order_product_query->rows as $product) {
					...
					$product_image = $this->db->query("SELECT image FROM `" . DB_PREFIX . "product` WHERE product_id = '" . (int)$product['product_id'] . "'");

					if ($product_image->row['image']) {
						$image = $this->model_tool_image->resize($product_image->row['image'], 140, 140);
					} else {
						$image = $this->model_tool_image->resize('placeholder.png', 140, 140);
					}
