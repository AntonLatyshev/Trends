...
class ModelCatalogProduct extends Model {
	...
	public function getProduct($product_id) {
		...
		$query = $this->db->query("SELECT DISTINCT *, p.product_id AS product_id_custom, pd.name AS name, p.image, m.name AS manufacturer,
...
(SELECT lcd.unit FROM " . DB_PREFIX . "length_class_description lcd WHERE p.length_class_id = lcd.length_class_id AND lcd.language_id = '" . (int)$this->config->get('config_language_id') . "') AS length_class,
		...
		LEFT JOIN " . DB_PREFIX . "category pcat
			ON (p2c.category_id = pcat.category_id)
		...
		if ($query->num_rows) {
			return array(
				'product_id'       => $query->row['product_id_custom'],
				'name'             => $query->row['name'],
	...
	public function getProducts($data = array()) {
		...
		if (!empty($data['filter_category_id'])) {
			$sql .= " LEFT JOIN " . DB_PREFIX . "category pcat ON (p2c.category_id = pcat.category_id) LEFT JOIN " . DB_PREFIX . "product_description pd ON (p.product_id = pd.product_id) LEFT JOIN " . DB_PREFIX . "product_to_store p2s ON (p.product_id = p2s.product_id) WHERE pd.language_id = '" . (int)$this->config->get('config_language_id') . "' AND p.status = '1' AND p.date_available <= NOW() AND p2s.store_id = '" . (int)$this->config->get('config_store_id') . "'";
		} else {
	...
	public function getBestSellerProducts($limit) {
		...
		if (!$product_data) {
			...
			$query = $this->db->query("SELECT op.product_id, SUM(op.quantity) AS total FROM " . DB_PREFIX . "order_product op LEFT JOIN `" . DB_PREFIX . "order` o ON (op.order_id = o.order_id) LEFT JOIN `" . DB_PREFIX . "product` p ON (op.product_id = p.product_id)
			LEFT JOIN " . DB_PREFIX . "product_to_store p2s
				ON (p.product_id = p2s.product_id)
	...
	public function getProductRelated($product_id) {
		...
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "product_related pr LEFT JOIN " . DB_PREFIX . "product p ON (pr.related_id = p.product_id)
    ...
    public function getCategoryByProduct($pr_id) {
        $query = $this->db->query("SELECT pc.category_id, pd.name FROM " . DB_PREFIX . "product_to_category pc "
            . "LEFT JOIN " . DB_PREFIX . "category_description pd ON (pd.category_id = pc.category_id) "
            . "WHERE pc.product_id = '".$pr_id."'");

        return $query->row;
    }

    public function getManufacturerByProduct($pr_id) {
        $query = $this->db->query("SELECT p.manufacturer_id, m.name FROM " . DB_PREFIX . "product p "
