...
class ControllerDesignBanner extends Controller {
	...
	protected function getForm() {
		...
		foreach ($banner_images as $banner_image) {
			...
			if (is_file(DIR_IMAGE . $banner_image['image_add'])) {
				$image_add = $banner_image['image_add'];
				$thumb_add = $banner_image['image_add'];
			} else {
				$image_add = '';
				$thumb_add = 'no_image.png';
			}
			...
			$data['banner_images'][] = array(
				...
				'thumb_add'                => $this->model_tool_image->resize($thumb_add, 100, 100),
		...
		if (isset($this->request->post['banner_image'])) {
			foreach ($this->request->post['banner_image'] as $banner_image_id => $banner_image) {
				foreach ($banner_image['banner_image_description'] as $language_id => $banner_image_description) {
					if ((utf8_strlen($banner_image_description['title']) < 2) || (utf8_strlen($banner_image_description['title']) > 256)) {
