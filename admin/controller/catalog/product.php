...
class ControllerCatalogProduct extends Controller {
	...
	public function edit() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			if ( !empty($this->request->post['custom_field']) ) {
				...
				$this->model_catalog_product->editCustomFields($this->request->get['product_id'], $json_custom_fields);
			} else {
	...
	protected function getForm() {
		...
		if (isset($this->request->post['custom_field'])) {
			...
			$custom_fields = $this->model_catalog_product->getPoductCustomFields($this->request->get['product_id']);
		...
		if (isset($this->request->post['image_bg'])) {
			...
			$data['image_bg'] = $product_info['image_bg'];
		} else {
			$data['image_bg'] = '';
		}
		...
		} elseif (!empty($product_info) && is_file(DIR_IMAGE . $product_info['image_bg'])) {
			$data['thumb_bg'] = $this->model_tool_image->resize($product_info['image_bg'], 100, 100);
		} else {
		...
		/*code start*/
		$data['promotags'] = $this->model_catalog_product->getPromoTags();

		if (isset($this->request->post['promo_top_right'])) {
			$data['promo_top_right'] = $this->request->post['promo_top_right'];
		} elseif (isset($product_info)) {
			$data['promo_top_right'] = $product_info['promo_top_right'];
		} else {
		...
		if (isset($this->request->post['promo_date_start'])) {
			...
			if($product_info['promo_date_start'] == '0000-00-00 00:00:00') {
				...
				$data['promo_date_start'] = $product_info['promo_date_start'];
			}
		} else {
			$data['promo_date_start'] = '';
		}
		...
		if (isset($this->request->post['promo_date_end'])) {
			...
			if($product_info['promo_date_end'] == '0000-00-00 00:00:00') {
				...
				$data['promo_date_end'] = $product_info['promo_date_end'];
			}
		} else {
			$data['promo_date_end'] = '';
		}
			...
			if (isset($product_option['product_option_value'])) {
				foreach ($product_option['product_option_value'] as $product_option_value) {
                    ...
                    if ($product_option_value['product_option_image'] && file_exists(DIR_IMAGE . $product_option_value['product_option_image'])) {
                        $opt_image = $product_option_value['product_option_image'];
                    } else {
                    ...
                    $product_option_value_data[] = array(
                        ...
                        'product_option_image'    	 => $opt_image,
                        'product_option_image_thumb' => $this->model_tool_image->resize($opt_image, 100, 100),
    ...
    public function fastUpdate() {
        ...
        if (($this->request->server['REQUEST_METHOD'] == 'POST') && !empty($this->request->post)){
            ...
            if(isset($this->request->post['name']) && isset($this->request->post['pk']) && isset($this->request->post['value'])) {
                $this->model_catalog_product->fastUpdate($this->request->post['name'], $this->request->post['pk'], $this->request->post['value']);
            }
        }
    }
    public function editProductSpecial() {
        if (($this->request->server['REQUEST_METHOD'] == 'POST') && !empty($this->request->post)){
            $this->load->model('catalog/product');
            if(isset($this->request->post['name']) && isset($this->request->post['pk']) && isset($this->request->post['value'])) {
                $s =  $this->model_catalog_product->priceFromListUpdate($this->request->post['pk'], $this->request->post['value']);
            }
        }
    }
