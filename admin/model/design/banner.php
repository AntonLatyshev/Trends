	...
	public function addBanner($data) {
		...
		if (isset($data['banner_image'])) {
			foreach ($data['banner_image'] as $banner_image) {
				$this->db->query("INSERT INTO " . DB_PREFIX . "banner_image SET banner_id = '" . (int)$banner_id . "', link = '" .  $this->db->escape($banner_image['link']) . "', image = '" .  $this->db->escape($banner_image['image']) . "', width = '" .  $this->db->escape($banner_image['width']) . "', height = '" .  $this->db->escape($banner_image['height']) . "', image_add = '" .  $this->db->escape($banner_image['image_add']) . "', width_add = '" .  $this->db->escape($banner_image['width_add']) . "', height_add = '" .  $this->db->escape($banner_image['height_add']) . "', sort_order = '" . (int)$banner_image['sort_order'] . "'");

				$banner_image_id = $this->db->getLastId();

				foreach ($banner_image['banner_image_description'] as $language_id => $banner_image_description) {
					$this->db->query("INSERT INTO " . DB_PREFIX . "banner_image_description SET banner_image_id = '" . (int)$banner_image_id . "', language_id = '" . (int)$language_id . "', banner_id = '" . (int)$banner_id . "', title = '" .  $this->db->escape($banner_image_description['title']) . "', text = '" .  $this->db->escape($banner_image_description['text']) . "', additional = '" .  $this->db->escape($banner_image_description['additional']) . "'");
				}
			}
		}
		...
		foreach ($banner_image_query->rows as $banner_image) {
			...
			foreach ($banner_image_description_query->rows as $banner_image_description) {
				$banner_image_description_data[$banner_image_description['language_id']] = array(
					...
					'description' 	=> $banner_image_description['text'],
					'additional' 	=> $banner_image_description['additional'],
				);
			}
			...
			$banner_image_data[] = array(
				...
				'image_add'                => $banner_image['image_add'],
				'width_add'                => $banner_image['width_add'],
				'height_add'               => $banner_image['height_add'],
