...
class ModelCatalogCategory extends Model {
	...
	public function addCategory($data) {
		...
		$this->db->query("INSERT INTO " . DB_PREFIX . "category SET parent_id = '" . (int)$data['parent_id'] . "', `bottom` = '" . (isset($data['bottom']) ? (int)$data['bottom'] : 0) . "', `top` = '" . (isset($data['top']) ? (int)$data['top'] : 0) . "', `column` = '" . (int)$data['column'] . "', sort_order = '" . (int)$data['sort_order'] . "', status = '" . (int)$data['status'] . "', customer_group_id = '" . (int)$data['customer_group_id'] . "', date_modified = NOW(), date_added = NOW()");
		...
		if (isset($data['image_bg'])) {
			$this->db->query("UPDATE " . DB_PREFIX . "category SET image_bg = '" . $this->db->escape($data['image_bg']) . "' WHERE category_id = '" . (int)$category_id . "'");
		}
	...
	public function editCategory($category_id, $data) {
		...
		$this->db->query("UPDATE " . DB_PREFIX . "category SET parent_id = '" . (int)$data['parent_id'] . "', `bottom` = '" . (isset($data['bottom']) ? (int)$data['bottom'] : 0) . "', `top` = '" . (isset($data['top']) ? (int)$data['top'] : 0) . "', `column` = '" . (int)$data['column'] . "', sort_order = '" . (int)$data['sort_order'] . "', status = '" . (int)$data['status'] . "', customer_group_id = '" . (int)$data['customer_group_id'] . "', date_modified = NOW() WHERE category_id = '" . (int)$category_id . "'");
	...
	public function getCategoryCustomFields($category_id) {
		$query = $this->db->query("SELECT custom_field FROM " . DB_PREFIX . "category WHERE category_id = '" . (int)$category_id . "'");

		return $query->row['custom_field'];
	}

	public function editCustomFields($category_id, $data) {
		//$this->event->trigger('pre.admin.category.edit', $data);

		$this->db->query("UPDATE " . DB_PREFIX . "category SET custom_field = '" . $this->db->escape($data) . "', date_modified = NOW() WHERE category_id = '" . (int)$category_id . "'");
