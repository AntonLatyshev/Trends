...
class ModelCatalogProduct extends Model {
    ...
    $this->db->query("DELETE FROM " . DB_PREFIX . "ocfilter_option_value_to_product WHERE product_id = '" . (int)$product_id . "'");
		...
		if (isset($data['image_bg'])) {
			$this->db->query("UPDATE " . DB_PREFIX . "product SET image_bg = '" . $this->db->escape($data['image_bg']) . "' WHERE product_id = '" . (int)$product_id . "'");
		}
		...
		foreach ($data['product_description'] as $language_id => $value) {
			$this->db->query("INSERT INTO " . DB_PREFIX . "product_description SET product_id = '" . (int)$product_id . "', language_id = '" . (int)$language_id . "', name = '" . $this->db->escape($value['name']) . "', description = '" . $this->db->escape($value['description']) . "', description_short = '" . $this->db->escape($value['description_short']) . "', tag = '" . $this->db->escape($value['tag']) . "', meta_title = '" . $this->db->escape($value['meta_title']) . "', meta_description = '" . $this->db->escape($value['meta_description']) . "', meta_keyword = '" . $this->db->escape($value['meta_keyword']) . "'");
		}
	...
	public function getPoductCustomFields($product_id) {
		$query = $this->db->query("SELECT custom_field FROM " . DB_PREFIX . "product WHERE product_id = '" . (int)$product_id . "'");

		return $query->row['custom_field'];
	}

	public function editCustomFields($product_id, $data) {
		//$this->event->trigger('pre.admin.product.edit', $data);

		$this->db->query("UPDATE " . DB_PREFIX . "product SET custom_field = '" . $this->db->escape($data) . "', date_modified = NOW() WHERE product_id = '" . (int)$product_id . "'");
	...
	public function getProducts($data = array()) {
        ...
        if (!empty($data['filter_category_id'])) {
            ...
            //echo "SELECT category_id FROM " . DB_PREFIX . "category_description WHERE name = '" . $data['filter_category'] . "' LIMIT 1";

            // $category_id = $this->db->query("SELECT category_id FROM " . DB_PREFIX . "category_description WHERE name = '" . $data['filter_category'] . "' LIMIT 1")->row['category_id'];
        ...
        if (!empty($data['filter_category_id'])) {
            $sql .= " AND pc.category_id = '" . $data['filter_category_id'] . "'";
        }
	...
	public function getTotalProducts($data = array()) {
        ...
        if (!empty($data['filter_category_id'])) {
            // $cat_id = $this->db->query("SELECT category_id FROM " . DB_PREFIX . "category_description WHERE name = '" . $data['filter_category_id'] . "' LIMIT 1")->row['category_id'];
            $sql .= " LEFT JOIN " . DB_PREFIX . "product_to_category pc ON (p.product_id = pc.product_id)";
        }
    ...
    public function fastUpdate($field, $product_id, $field_value) {
        switch ($field) {
            ...
            case 'product_sort_order':
                $sql = "Update " . DB_PREFIX . "product Set sort_order = " . (int)$this->db->escape($field_value) . " Where product_id = " . (int)$product_id;
                break;
            case 'product_status':
                $sql = "Update " . DB_PREFIX . "product Set stock_status_id = " . (int)$this->db->escape($field_value) . " Where product_id = " . (int)$product_id;
                break;
            case 'product_promo_tag':
                $sql = "Update " . DB_PREFIX . "product Set promo_top_right = " . (int)$this->db->escape($field_value) . " Where product_id = " . (int)$product_id;
                break;
            case 'percent_special':
                $sql = "Update " . DB_PREFIX . "product_special Set percent = " . (int)$this->db->escape($field_value) . " Where product_id = " . (int)$product_id;
                break;
            case 'product_on':
